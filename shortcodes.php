...
function wp_nasaads_query_importer_shortcode($atts = [], $template = '', $tag = '') {
    if (esc_html(get_option('wp_nasaads_query_importer-valid_token')) != 1) {
    ...
    $atts = shortcode_atts(array(
        ...
        'max_authors' => 3,
        'notify_empty_list' => esc_html(get_option('wp_nasaads_query_importer-empty_list'))
                               ? 'true' : 'false',
    ...
    if ($template === '') {
        $shorttemp = false;
        $template = esc_attr(get_option('wp_nasaads_query_importer-template'));
    }
    ...
    if (sizeof($html) > 0) {
        $html = ($shorttemp ? '<p>'
                 : esc_attr(get_option('wp_nasaads_query_importer-template_start')))
              . implode("</p>\n<p>", $html)
              . ($shorttemp ? '</p>'
                 : esc_attr(get_option('wp_nasaads_query_importer-template_stop')));
    } else {
        $html = '';
    ...
    # acknowledge ADS
    if (esc_attr(get_option('wp_nasaads_query_importer-acknowledge'))) {
